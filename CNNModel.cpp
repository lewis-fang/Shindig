...
bool CNNModel::LaunchCNNModelBySimdTrian(image image0, float* outValues, int outLentgh, int b)
{
	bool ret = true;
	if (CNNLayerSeries.size() == 0)
	{
	...
	{
		CNNCalc& layer = CNNLayerSeries.at(ly);
		if (layer.LaunchConvolutionBySimdBN(train,b))
		{
			tempIamge = layer.getOutImage();
			if (ly != (depth - 1))
			{
...
void CNNModel::initMemoryV2(int batch)
{
	int i = 0;
	for (CNNCalc& layer : CNNLayerSeries)
	{
		layer.initLayerMemoryV2(batch);
		int reslinkPre = layer.getResLink();
		if (reslinkPre  > -1)
		{
		...
		{
			CNNCalc& bLayer = CNNLayerSeries.at(i-1);
			memcpy(&(layer.getCurrentLayerIdealInoutDxdy()), &(bLayer.getCurrentLayerIdealOutDxdy()), sizeof(image));
		}
		i++;
	}
...
bool CNNModel::LaunchCNNModelParrallel(std::vector<image> imagen, float* outValues, int outLentgh, int batch)
{
	...
	for (image image0 : imagen)
	{
		...
		{
			thr[b] = std::thread(&CNNModel::LaunchCNNModelBySimdBatch, this, image0, outValues+ cnt* outLentgh*batch, outLentgh, batch, b);
			thr[b].detach();
		}
		cnt++;
...
void CNNModel::updateLossParrallel(image bImage, float* outValue, float* vC, int outLen, int realIndex, int b)
{
	...
	if (ret == true)
	{
		...
		int reslink = -1;
		image resDbzactImage;
		bool waitForPreLayer = false;
		for (int ly = dpth - 1; ly >= 0; ly--)
		{
			CNNCalc& layer = CNNLayerSeries.at(ly);
			ret = layer.UpdateLayerLossSimd(b);
			image DeltaImage = layer.getCurrentLayerIdealInoutDxdy();		
			if (reslink == -1)
			{
				reslink = layer.getResLink();
				resDbzactImage = layer.getDbzActImage();
			}		
			if (ly == reslink)
			{
